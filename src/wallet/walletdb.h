...
public:
  ...
  //! write the hdchain model (external chain child index counter)
    ...
    bool WritezkuboSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadzkuboSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadzkuboSeed_deprecated(uint256& seed);
    bool ErasezkuboSeed();
    bool ErasezkuboSeed_deprecated();

    bool WritezkuboCount(const uint32_t& nCount);
    bool ReadzkuboCount(uint32_t& nCount);
