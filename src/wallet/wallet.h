...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zkubospends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 KUBO at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zkubo send
enum ZerocoinSpendStatus {
    zkubo_SPEND_OKAY = 0,                            // No error
    zkubo_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    zkubo_WALLET_LOCKED = 2,                         // Wallet was locked
    zkubo_COMMIT_FAILED = 3,                         // Commit failed, reset status
    zkubo_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    zkubo_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    zkubo_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    zkubo_TRX_CREATE = 7,                            // Everything related to create the transaction
    zkubo_TRX_CHANGE = 8,                            // Everything related to transaction change
    zkubo_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    zkubo_INVALID_COIN = 10,                         // Selected mint coin is not valid
    zkubo_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    zkubo_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    zkubo_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    zkubo_SPENT_USED_zkubo = 14,                      // Coin has already been spend
    zkubo_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    zkubo_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void zkuboBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreatezkuboOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzkuboAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzkuboTracker> zkuboTracker;
    ...
    void setZWallet(CzkuboWallet* zwallet)
    {
        zwalletMain = zwallet;
        zkuboTracker = std::unique_ptr<CzkuboTracker>(new CzkuboTracker(strWalletFile));
    }

    CzkuboWallet* getZWallet() { return zwalletMain; }
    ...
    void setzkuboAutoBackups(bool fEnabled)
    {
    ...
    /// Get 5000KUBO input and keys which can be used for the Masternode
    ...
    /** zkubo reset */
    boost::signals2::signal<void()> NotifyzkuboReset;
