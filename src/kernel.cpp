...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CzkuboStake(spend));
    } else {
        ...
        CKUBOStake* KUBOInput = new CKUBOStake();
        KUBOInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(KUBOInput);
    }
