...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total kubocoin balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zkubosupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zkubo denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zkubo denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zkubo denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zkubo denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zkubo denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zkubo denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zkubo denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zkubo denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zkubo denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in kubocoin/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in kubocoin/kb\n"
    ...
    UniValue zkuboObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zkuboObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zkuboObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("zkubosupply", zkuboObj));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"kubocoinaddress\"\n"
            "\nReturn information about the given kubocoin address.\n"

            "\nArguments:\n"
            "1. \"kubocoinaddress\"     (string, required) The kubocoin address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"kubocoinaddress\", (string) The kubocoin address validated\n"
...
UniValue createsporkkeypair(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"kubocoinaddr\" : \"kubocoinaddress\", (string) The kubocoin public address \n"
    ...
    if (isValid) {
        ...
        CTxDestination dest = address.Get();
        //get public address
        ret.push_back(Pair("kubocoinaddr", currentAddress));
        CScript scriptPubKey = GetScriptForDestination(dest);
        //get scriptpubkey
...
#ifdef ENABLE_WALLET
        // Case 1: kubocoin address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are kubocoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) kubocoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"kubocoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"kubocoinaddress\"  (string, required) The kubocoin address to use for the signature.\n"
