...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zkuboTracker->HasMintTx(hash))) {
            //zkubo stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::Stakezkubo;
        ...
        for (const CTxOut& txout : wtx.vout) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecord sub(hash, nTime);
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                sub.type = TransactionRecord::ZerocoinSpend_Change_zkubo;
                sub.address = mapValue["zerocoinmint"];
                if (!fFeeAssigned) {
...
bool IszkuboType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::Stakezkubo:
        case TransactionRecord::ZerocoinMint:
        case TransactionRecord::ZerocoinSpend:
        case TransactionRecord::RecvFromZerocoinSpend:
        case TransactionRecord::ZerocoinSpend_Change_zkubo:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::Stakezkubo || type == TransactionRecord::MNReward) {
