...
#include "ui_zkubocontroldialog.h"
...
std::set<std::string> zkuboControlDialog::setSelectedMints;
std::set<CMintMeta> zkuboControlDialog::setMints;

bool CzkuboControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == zkuboControlDialog::COLUMN_DENOMINATION || column == zkuboControlDialog::COLUMN_VERSION || column == zkuboControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


zkuboControlDialog::zkuboControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::zkuboControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
zkuboControlDialog::~zkuboControlDialog()
{
    delete ui;
}

void zkuboControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void zkuboControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CzkuboControlWidgetItem* itemDenom(new CzkuboControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CzkuboControlWidgetItem *itemMint = new CzkuboControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zkuboTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zkuboTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zkubo.";
            else if (!mint.isSeedCorrect)
                strReason = "The zkubo seed used to mint this zkubo is not the same as currently hold in the wallet";
            else
...
void zkuboControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void zkuboControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelzkubo_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setzkuboControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> zkuboControlDialog::GetSelectedMints()
{
...
void zkuboControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
