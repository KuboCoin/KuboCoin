                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zkubo ought to be enough for anybody." - Bill Gates, 2017
    ui->zkubopayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzkuboSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zkubopayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzkubo_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zkubo is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zkubo, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zkubo...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zkubo in ") +
...
void PrivacyDialog::on_pushButtonSpendzkubo_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zkubo is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zkubo, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zkubo
        sendzkubo();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zkubo
    sendzkubo();
}

void PrivacyDialog::on_pushButtonzkuboControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    zkuboControlDialog* zkuboControl = new zkuboControlDialog(this);
    zkuboControl->setModel(walletModel);
    zkuboControl->exec();
}

void PrivacyDialog::setzkuboControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzkuboSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzkubo()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid kubocoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zkubopayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zkubo is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " KUBO </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zkubo</b>";
    ...
    // use mints from zkubo selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!zkuboControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = zkuboControlDialog::GetSelectedMints();
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zkubo was spent successfully update the addressbook with the label
    ...
    // Clear zkubo selector in case it was used
    zkuboControlDialog::setSelectedMints.clear();
    ui->labelzkuboSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zkubo Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " KUBO, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zkubo Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zkuboTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zkubo </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zkubo "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zkubo "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zkubo "));
    ui->labelzkuboAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zkubo </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zkubo </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zkubo </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzkuboSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzkubo->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zkubo
        ui->pushButtonMintzkubo->setEnabled(false);
        ui->pushButtonMintzkubo->setToolTip(tr("zkubo is currently disabled due to maintenance."));

        // Spend zkubo
        ui->pushButtonSpendzkubo->setEnabled(false);
        ui->pushButtonSpendzkubo->setToolTip(tr("zkubo is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zkubo
        ui->pushButtonMintzkubo->setEnabled(true);
        ui->pushButtonMintzkubo->setToolTip(tr("PrivacyDialog", "Enter an amount of KUBO to convert to zkubo", 0));

        // Spend zkubo
        ui->pushButtonSpendzkubo->setEnabled(true);
        ui->pushButtonSpendzkubo->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
