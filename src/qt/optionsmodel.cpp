...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::KUBO);
    ...
    if (!settings.contains("nAnonymizekubocoinAmount"))
        settings.setValue("nAnonymizekubocoinAmount", 1000);

    nAnonymizekubocoinAmount = settings.value("nAnonymizekubocoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizekubocoinAmount"))
        SoftSetArg("-anonymizekubocoinamount", settings.value("nAnonymizekubocoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in kubocoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizekubocoinAmount:
            return QVariant(nAnonymizekubocoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizekubocoinAmount:
            nAnonymizekubocoinAmount = value.toInt();
            settings.setValue("nAnonymizekubocoinAmount", nAnonymizekubocoinAmount);
            emit anonymizekubocoinAmountChanged(nAnonymizekubocoinAmount);
            break;
        case CoinControlFeatures:
