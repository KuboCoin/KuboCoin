...
#include "zkubo/accumulatormap.h"
#include "addrman.h"
#include "alert.h"
...
#if defined(NDEBUG)
#error "kubocoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uKUBO) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zkubo spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zkubo is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend.HasValidSignature())
                return error("%s: V2 zkubo spend does not have a valid signature\n", __func__);
        ...
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zkubo without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (!spend.HasValidSerial(Params().Zerocoin_Params(fUseV1Params))) {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zkubo spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zkubo spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zkubo");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zkubo mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	nSubsidy = 0 * COIN;
        ...
        nSubsidy = 8970000000 * COIN; //Pre-mine blocks 1-350 89,700,000,000 (89.7B for swap fund from erc20 token to main chain)
	} else if (nHeight <= 10 && nHeight >= 2) {
        nSubsidy = 8970000000 * COIN;
	} else if (nHeight <= Params().LAST_POW_BLOCK() && nHeight > 11) { //end PoW
        nSubsidy = 18550 * COIN;
	} else if (nHeight <= 350400 && nHeight > Params().LAST_POW_BLOCK()) { //Start PoS
        nSubsidy = 18550 * COIN;
	} else if (nHeight <= 3364216 && nHeight >= 700801) {
        nSubsidy = 3567 * COIN;
	}    else {
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool iszkuboStake){
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from kubocoin
...
{
    RenameThread("kubocoin-scriptch");
...
void RecalculatezkuboMinted()
{
    CBlockIndex *pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating minted zkubo..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted zkubo..."), percent);
        }
...
void RecalculatezkuboSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent zkubo..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent zkubo..."), percent);
        }

        //Rewrite zkubo supply
        CBlock block;
        assert(ReadBlockFromDisk(block, pindex));
        ...
        //Remove spends from zkubo supply
...
bool RecalculateKUBOSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating KUBO supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating KUBO supply..."), percent);
        }
...
{
    // kubocoin: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdatezkuboSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zkubo supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    bool fEnforceBIP30 = (!pindex->phashBlock) || // Enforce on CreateNewBlock invocations which don't have a hash.
                         !((pindex->nHeight == 0 && pindex->GetBlockHash() == uint256("0x000008d67dbdf5f9bb9fba21d36e90d31a0fac2235420faf57b949fe97937dee")));
			   //||
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculatezkuboMinted();
        RecalculatezkuboSpent();
        RecalculateKUBOSupply(Params().Zerocoin_StartHeight());
    }

    //Track zkubo money supply in the block index
    if (!UpdatezkuboSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zkubo supply for block=%s height=%d", __func__,
   ...
   // Unspendable Value (coins burn) can cause a negative nMint value
    if (pindex->nMint < 0)
        pindex->nMint = 0;
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zkuboSpent: %s\n",
...
/*
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert KUBO to zkubo
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zkubo spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zkubo serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zkubo stake block is less than allowed start height", __func__);

    if (CzkuboStake* zkubo = dynamic_cast<CzkuboStake*>(stake)) {
        CBlockIndex* pindexFrom = zkubo->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zkubo stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zkubo stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zkubo->GetValue()));
        if (nChecksum200 != zkubo->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zkubo->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->Iszkubo() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zkubo fails context checks", __func__));
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> kuboInputs;
        std::vector<CTxIn> zkuboInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.scriptSig.IsZerocoinSpend()){
                zkuboInputs.push_back(stakeIn);
            }else{
                kuboInputs.push_back(stakeIn);
            }
        }
        const bool hasKUBOInputs = !kuboInputs.empty();
        const bool haszkuboInputs = !zkuboInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasKUBOInputs)
                    ...
                    for (const CTxIn& KUBOIn : kuboInputs){
                        if(KUBOIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction &t : bl.vtx) {
                    for (const CTxIn &in: t.vin) {
                        ...
                        for (const CTxIn &stakeIn : kuboInputs) {
            ...
            // Now that this loop if completed. Check if we have zkubo inputs.
            if(haszkuboInputs){
                for (const CTxIn& zkuboInput : zkuboInputs) {
                    CoinSpend spend = TxInToZerocoinSpend(zkuboInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zkubo");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zkubo mints and %d zkubo spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // kubocoin: We use certain sporks during IBD, so check to see if they are
      ...
      // cleanSubVer has /KuboCoin.../ for us
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zkubo", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zkubo", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
