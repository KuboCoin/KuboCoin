...
CzkuboWallet::CzkuboWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zkubo seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadzkuboSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.ErasezkuboSeed_deprecated()) {
                    LogPrintf("%s: Updated zkubo seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zkubo seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zkubo wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzkuboWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WritezkuboCount(nCountLastUsed);
    else if (!walletdb.ReadzkuboCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzkuboWallet::Lock()
{
    seedMaster = 0;
}

void CzkuboWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzkuboWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedTozkubo(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzkuboWallet::LoadMintPoolFromDB()
{
...
void CzkuboWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzkuboWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzkuboWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zkuboTracker->HasPubcoinHash(pMint.first)) {
...
bool CzkuboWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedTozkubo(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zkuboTracker which also adds to database
    pwalletMain->zkuboTracker->Add(dMint, true);
...
void CzkuboWallet::SeedTozkubo(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzkuboWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzkuboWallet::GenerateDeterministiczkubo(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzkuboWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzkuboWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzkuboWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
