// Copyright (c) 2018 The kubocoin developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zkubo/deterministicmint.h>
#include "zkubotracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzkuboTracker::CzkuboTracker(std::string strWalletFile)
{
...
CzkuboTracker::~CzkuboTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzkuboTracker::Init()
{
...
bool CzkuboTracker::Archive(CMintMeta& meta)
{
...
bool CzkuboTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzkuboTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzkuboTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzkuboTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzkuboTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzkuboTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzkuboTracker::GetSerialHashes()
{
...
CAmount CzkuboTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzkuboTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzkuboTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzkuboTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzkuboTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzkuboTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzkuboTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzkuboTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzkuboTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzkuboTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzkuboTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzkuboWallet* zkuboWallet)
{
    bool iszkuboWalletInitialized = (NULL != zkuboWallet);
    CMintMeta meta;
    ...
    if (! iszkuboWalletInitialized)
        zkuboWallet = new CzkuboWallet(strWalletFile);
    meta.isSeedCorrect = zkuboWallet->CheckSeed(dMint);
    if (! iszkuboWalletInitialized)
        delete zkuboWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzkuboTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzkuboTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzkuboTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzkuboTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzkuboTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzkuboTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzkuboWallet* zkuboWallet = new CzkuboWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zkuboWallet);
        }
        delete zkuboWallet;
        LogPrint("zero", "%s: added %d dzkubo from DB\n", __func__, listDeterministicDB.size());
    }
